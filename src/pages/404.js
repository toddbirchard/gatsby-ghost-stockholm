  ...
  return (
    <>
      <Layout template="error-template">
        ...
        <main>
          <h2 className="trending-title">Having trouble? Here are some of our trending posts:</h2>
          <div className="trending-posts">
            {topPages.map(({ node }) => (
              <div className="post-card" key={node.id}>
                <Link to={node.url}>
                  <picture className="feature-image">
                    {node.feature_image && <img className="post-card-image lazyload" data-src={node.feature_image} alt={node.title} title={node.title} />}
                  </picture>
                </Link>
  ...
  <StaticQuery
    query={graphql`
      query PopularPostsQuery {
        allMysqlWeeklyPostAnalytics(sort: {fields: views, order: DESC}, limit: 8) {
          edges {
            node {
              id
              title
